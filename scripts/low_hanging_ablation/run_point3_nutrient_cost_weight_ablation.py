#!/usr/bin/env python3
"""Point 3: fertilization nutrient-cost-weight ablation (parallel launcher)."""
...
DEFAULT_BASE_OUTPUT = REPO_ROOT / "artifacts" / "final_successful_runs" / "low_hanging_ablation" / "point3_nutrient_cost_weight"
            ...
            for fixed_weather in weather_modes:
                for nonadaptive in adaptive_modes:
                    for cost_weight in cost_weights:
                        ...
                        weight_label = slugify(f"{cost_weight:.6f}".rstrip("0").rstrip("."))
                        slug = (
                            f"p3_{method.lower()}_{adaptive_label}_{weather_label}_"
                            f"seed{seed}_costw{weight_label}"
                        )
                        ...
                        label = (
                            f"Point3 CostWeight | {method} | {adaptive_label} | {weather_label} | "
                            f"seed={seed} | nutrient_cost_weight={cost_weight}"
                        )
                        cases.append(
                            AblationCase(
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run Point-3 nutrient-cost-weight ablation in parallel.")
    ...
    parser.add_argument(
        "--cost-weights",
        default="0.8,1.0,1.2",
        help="Comma-separated nutrient-cost weights.",
    )
    ...
    cost_weights = [max(0.0, w) for w in parse_float_list(args.cost_weights)]
